...
//
// if something has a license or source reference right above it
//      - whatever license it says, or whatever license the original source says
//
// everything else:
//      - public domain
...
dialogOpen = false
openDialog = function (content, center) {
...
closeDialog = cancelDialog = function () {
}
...
randomIdentifier = function (size, chars) {
    if (!chars) {
    ...
    for (var i = 0; i < size; i++) {
        var cs = chars || (i == 0 ? firstChars : nextChars)
        s.push(cs[randomIndex(cs.length)])
    }
    return s.join('')
}
